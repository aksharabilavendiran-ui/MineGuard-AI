...
app.post('/api/auth/login', (req, res) => {
  ...
  const normalizedRole = requestedRole === 'planner' || requestedRole === 'regulator' || requestedRole === 'admin' ? requestedRole : 'operator';

  if (email !== 'virtualthrone@sih.in' || password !== 'virtualthrone123') {
    return res.status(401).json({ error: 'Invalid demo credentials' });
  }

  const user = { name: 'AKSHARA B', email, role: normalizedRole };
  return res.json({ token: jwt.sign(user, secret, { expiresIn: '8h' }), user });
});
